...
function ModalExpediente({ alumno, onClose }) {
    ...
    return ( <
        ...
        strong style = {
            {
                color: prom >= 8.5 ? "#16a34a" : prom >= 7 ? "#d97706" : "#dc2626",
            }
        } >
        ...
        onClick = {
            () => toast.success("Boleta PDF lista para imprimir") } >
        📄Descargar Boleta <
        /button> <
        /div> <
        /div> <
...
function ModalFormAlumno({ alumno, grupos, onClose, onSave }) {
    ...
    return ( <
        ...
        div className = "modal-header" >
        <
        div className = "modal-title" > { alumno ? "✏️ Editar Alumno" : "👨‍🎓 Nuevo Alumno" } <
        /div> <
        button className = "btn btn-ghost btn-icon"
        onClick = { onClose } > ✕
        <
        ...
        <
        button className = "btn btn-primary"
        onClick = { handleSubmit } > { alumno ? "💾 Guardar Cambios" : "➕ Registrar Alumno" } <
        /button> <
        /div> <
        /div> <
        /div>
